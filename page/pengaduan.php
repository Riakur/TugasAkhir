<div class="typography" style="background-color:#f3c18e8c;">
				<div class="container">
					...
					<h3 class="hdg">Pengaduan Saya</h3>
					...
					<div class="bs-docs-example wow fadeInUp animated" data-wow-delay=".5s" style="background-color:#18b9e98c;">
						<table class="table table-striped" border="1">
							<thead>
								...
								<?php 
								$no=1;
								$pe=$koneksi->query("SELECT * FROM pengaduan  where id_user='$_SESSION[id]'   order by tgl_pengaduan desc");
            ...
            $mbi=mysqli_fetch_array($bidan);
								...
								<tr>
									...
									<td><?php if($peng['status']=='Sedang di Proses Oleh Admin'){?>
                                    ...
                                    <?php }elseif($peng['status']=='di Tolak'){?>
												<a href="#" data-toggle="modal" data-target="#pengaduan<?php echo $peng['id_pengaduan'];?>" class="btn btn-success btn-circle btn-sm">
                                        <i class="fa fa-check"></i>
                                    </a>
                                     <a href="?page=page/hapuspengaduan&id=<?php echo $peng['id_pengaduan'];?>" class="btn btn-danger btn-circle btn-sm" onclick="return confirm ('Apakah anda yakin untuk menghapus data ini ?');" >
                                        <i class="fa fa-trash"></i>
